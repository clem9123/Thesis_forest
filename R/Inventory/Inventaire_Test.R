...
for (i in 1:nrow(combinations)) {
  ...
  results <- generate_inventory(patch_id, distribution, species_proportion)
  
  # Formatter l'inventaire pour forceps
  forceps_inv <- forceps_format(results$inventory, patcharea)
  ...
  file_inventory(forceps_inv %>% select(-c(distribution, species_proportion, repetition)), patcharea, patchnumber, output_file)
}
...
potential_species <- 
  data.frame(species = results$potential_species) %>%
