...
# Load spatial data
uep <- st_read("C:/Users/cdemontgolf/Desktop/RETZ_FIF/uep.shp")

# Load attribute data
uep_data <- read.csv("data/BDD_UEP_2012.csv")
...
uep_data <- uep_data %>%
  left_join(uep %>% select(IIDT_UEP, geometry), by = c("Identifiant.peuplement.élémentaire" = "IIDT_UEP"))

# Clean column names
colnames(uep_data) <- gsub("....du.couvert.boisé.", "", colnames(uep_data))

# Remove NAs
uep_data <- uep_data %>% filter(!is.na(Identifiant.peuplement.élémentaire) & Identifiant.peuplement.élémentaire != "")

# Convert diameter classes to numeric
uep_data <- uep_data %>%
  mutate(`Diametre dominant` = case_when(
    Classe.de.catégorie.de.diamètre.dominant == "S" ~ 1.5,
    Classe.de.catégorie.de.diamètre.dominant == "E" ~ 8,
    Classe.de.catégorie.de.diamètre.dominant == "I" ~ NA_real_,
    Classe.de.catégorie.de.diamètre.dominant == "1" ~ 12.5,
    Classe.de.catégorie.de.diamètre.dominant == "P" ~ 27.5,
    Classe.de.catégorie.de.diamètre.dominant == "M" ~ 35.5,
    Classe.de.catégorie.de.diamètre.dominant == "G" ~ 57.5,
    Classe.de.catégorie.de.diamètre.dominant == "G-" ~ 50,
    Classe.de.catégorie.de.diamètre.dominant == "G+" ~ 65,
    Classe.de.catégorie.de.diamètre.dominant == "T" ~ 80
  ))

# Order factor levels
especes <- colnames(uep_data)[9:36]
uep_data <- uep_data %>%
  ...
  mutate(Classe.de.catégorie.de.diamètre.dominant = factor(Classe.de.catégorie.de.diamètre.dominant,
      levels = c("I", "S", "E", "1", "P", "M", "G", "G-", "G+", "T"),
      ordered = TRUE))
```

```{r}
ggplot(uep_data %>% mutate(Structure.et.occupation.du.sol = factor(Structure.et.occupation.du.sol, levels = c("F", "I", "R", "S", "T", "V", "Z", "M")))) +
...
```{r}
# supprimer les espaces non boisés
uep_data <- uep_data %>% filter(! Structure.et.occupation.du.sol %in% c("R", "V", "M"))
```
...
uep_data <- uep_data %>%
  ...
  separate(Classe.d.âge.en.2010, into = c("age_min", "age_max"), sep = "-", convert = TRUE, remove = FALSE) %>%
...
p2 <- ggplot(data = uep_data %>% filter(!is.na(Classe.d.âge.en.2010))) +
    geom_density(aes(x = surf.terrière.moy, fill = surf.terrière.moy, color = factor(Classe.d.âge.en.2010))) +
    theme_minimal() +
...
map_plot(uep_data, "Essence.dominante.1")
```

## Type de peuplement
```{r map type_peuplement, echo=FALSE, fig.width = 12, fig.height = 6}
palette_rainbow <- rainbow((length(unique(uep_data$Type.de.peuplement))))
type_peuplement_colors <- data.frame(palette_rainbow, sort(unique(uep_data$Type.de.peuplement)))
colnames(type_peuplement_colors) <- c("Color", "Type.de.peuplement")
p1 <- ggplot(uep_data %>% left_join(type_peuplement_colors)) +
...
p2 <- ggplot(uep_data %>% left_join(type_peuplement_colors), aes(x = Type.de.peuplement)) +
...
uep_data %>%
  ...
  summarise(Prop = n()/nrow(uep_data)) %>%
  arrange(-Prop) %>%
  mutate(Cum_Prop = cumsum(Prop)) %>%
  left_join(type_peuplement_colors) %>%
  filter(Cum_Prop <= 0.8) -> uep_data_80
```

Il y a `r nrow(uep_data_80)` peuplements qui représentent 80% des placettes.

```{r map type_peuplement_80_plot, echo=FALSE, fig.width = 6, fig.height = 6}
type_peuplement_colors <- type_peuplement_colors %>%
  filter(Type.de.peuplement %in% uep_data_80$Type.de.peuplement)

ggplot(uep_data_80, aes(x = Type.de.peuplement)) +
...
map_plot(uep_data, "Classe.de.catégorie.de.diamètre.dominant")
```
...
p2 <- ggplot(data = uep_data %>% rowwise() %>% mutate(CH = sum(c(CHS, CHP), na.rm = TRUE)) %>% ungroup()) +
...
p2 <- ggplot(uep_data, aes(x = Essence.dominante.1)) +
  geom_bar(aes(y = (..count..), fill = Essence.dominante.1)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) +
  theme(legend.position = "none") +
...
Il y a `r length(essence.dominante)` essences qui sont dominantes dans au moins une unité de gestion.
Ce sont : `r paste(unique(essence.dominante), collapse = ", ")`

## Surface terrière

```{r surface terrière, echo=FALSE, fig.width = 12, fig.height = 6}
ug_surf.terrière <- uep_data %>%
  summarize(
...
surf_tot = sum(uep_data$surface.retenue)
...
uep_data %>% select(ess.obj.SIG) %>% table() %>% as.data.frame() %>% 
...
surface_rare = uep_data %>% filter(Essence.dominante.1 %in% essence_prb) %>% pull(surface.retenue) %>% sum()
placette_rare = uep_data %>% filter(Essence.dominante.1 %in% essence_prb) %>% nrow()
surface_tot = uep_data %>% pull(surface.retenue) %>% sum()
placette_tot = uep_data %>% nrow()
```
