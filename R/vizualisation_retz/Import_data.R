...
# Contains geometry information for each forest stand (UEP = Unité Élémentaire de Peuplement)
spatial_coord <- 
...
# Classes: S=Semis, E=Étalé, I=Irrégulier, 1-9=diameter classes, P=Petit, M=Moyen, G=Gros, T=Très gros
table_ccd <- data.frame(
  Classe.de.catégorie.de.diamètre.dominant = 
    c("S", "E", "I", "1", "P", "M", "G", "G-", "G+", "T"),
  diamètre.min = c(2, 5, 7.5, 7.5, 17.5, 27.5, 47.5, 47.5, 55, 67.5),     # Minimum diameter (cm)
  diamètre.max = c(5, 10, 90, 17.5, 27.5, 47.5, 67.5, 55, 67.5, 100),      # Maximum diameter (cm)
  diamètre.mean = c(3.5, 7.5, 45, 12.5, 22.5, 37.5, 57.5, 52.5, 62.5, 85)) # Mean diameter (cm)

# Load pedological (soil) data from shapefile
# RUM = Réserve Utile Maximale (Maximum Available Water Capacity)
pedo_data <- 
...
# Remove "....du.couvert.boisé." suffix from species column names
...
forest_data <- forest_data %>%
    mutate(
        ...
        surf.terrière.moy = as.numeric(surf.terrière.moy),  # Mean basal area
        Age.réel.en.2010 = as.numeric(Âge.réel.en.2010),   # Real age in 2010
        across(all_of(especes), as.numeric)                # All species percentage columns
    ) %>%
    select(-Âge.réel.en.2010)  # Remove original age column (will be replaced by processed version)
         
# Process age class information
# Convert age class ranges (e.g., "20-40") to separate min/max columns and calculate median
forest_data <- forest_data %>% 
    mutate(
        # Handle missing values coded as "NC" (Non Classé = Not Classified)
...
# Join by forest stand ID (UEP = Unité Élémentaire de Peuplement)
forest_data <- forest_data %>%
  left_join(pedo_data,
